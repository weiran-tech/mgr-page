...
class ListPamToken extends ListBase
{
    ...
    public function columns()
    {
        ...
        $this->addColumn(Column::NAME_ACTION, '操作')->displayUsing(Actions::class, [function (Actions $actions) {
            /** @var PamToken $item */
            $item = $actions->row;
            $actions->request('下线用户', route('wr-mgr-page:backend.pam.delete_token', [$item->id]))->icon('phone-flip')
                ->confirm('使用户下线, 用户可重新登录')->primary();
            $actions->request('禁用设备', route('wr-mgr-page:backend.pam.ban', [$item->id, 'device']))->icon('phone')
                ->confirm('禁用此设备, 此设备无法再继续访问, 如需开启在黑名单中移除即可')->primary();
            $actions->request('禁用IP', route('wr-mgr-page:backend.pam.ban', [$item->id, 'ip']))->icon('reception-4')
